...
func (n *Node) Value() []byte {
	...
	case tLeaf:
		return n.value
	case tNode:
		return slices.Concat(n.left.Value(), n.right.Value())
	}
	panic("unreachable")
}
...
func (n *Node) Insert(pos int, value []byte) {
	...
	case tLeaf:
		// slice tricks insert
		n.value = slices.Insert(n.value, pos, value...)
		n.length = len(n.value)
	case tNode:
		leftLength := n.left.length
		if pos < leftLength {
		...
		if leftStart != leftEnd {
			if rightStart != rightEnd {
				return slices.Concat(
					n.left.Slice(leftStart, leftEnd),
					n.right.Slice(rightStart, rightEnd),
				)
			} else {
...
func (n *Node) Rebuild() {
	switch n.kind {
	case tNode:
		n.value = slices.Concat(n.left.Value(), n.right.Value())
		n.left = nil
		n.right = nil
		n.adjust()
	}
